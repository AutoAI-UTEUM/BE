...
공개 계정 역할은 **학습자(LEARNER)**와 **강사(INSTRUCTOR)**입니다. 이번 범위에서 두 역할은 같은 학습 기능과 소유권 규칙을 사용하며, 강사 전용 기능과 차등 권한은 후속 계약에서 정의합니다. 자료와 사용자를 관리하는 **관리자(ADMIN)**는 MVP의 보조 사용자입니다.
...
MVP에서 제외하거나 후순위로 두는 기능은 실시간 화상 강의, 강사-학습자 실시간 채팅, 결제, 복잡한 관리자 통계, 모든 LLM 출력의 완전 자동 사실 검증입니다. `INSTRUCTOR` 계정 역할은 도입하지만 Course, Lecture, Assignment, Notification과 강사 전용 기능·차등 권한은 후속 계약에서 별도 범위로 확정합니다.
