...
_GENERATE_NARRATIVE_INSTRUCTION = (
    ...
    "서술하고 recommendedActions는 실행 가능한 행동 단위로 작성하라. 각 narrative는 "
    "반드시 세 요소를 담아라: ① 관찰 — 연결한 evidence의 label과 fact를 구체적으로 "
    "인용, ② 해석 — 그 관찰이 rubric 관점에서 무엇을 의미하는지 한 문장, ③ 지도 "
    "포인트 — 교사가 다음 수업이나 면담에서 바로 할 수 있는 행동 한 가지. 2~4문장으로 "
    "쓰되 세 요소 중 하나라도 빠진 narrative를 내지 마라. 나쁜 예(금지): '질문 "
    "구체성이 보통 수준입니다.', '정답률이 60%입니다.' — 수치나 등급의 재진술만으로는 "
    "narrative가 아니다. 좋은 예(방향): '3주차 자료의 역전파 관련 질문 4건이 모두 "
    "용어 정의 확인에 머물렀습니다. rubric 기준으로 개념 간 연결 단계로 아직 나아가지 "
    "못한 상태입니다. 다음 수업에서 간단한 수치 예시를 직접 계산하게 해 보시면 연결 "
    "여부를 확인할 수 있습니다.' 이 예시는 구조와 문체를 위한 스타일 앵커일 뿐이며, "
    "실제 출력에서는 요청에 제공된 evidenceId, label, fact만 사용하라. "
    "recommendedActions의 각 항목은 대상 개념 또는 자료 위치와 교사의 행동이 함께 든 "
    "한 문장으로 쓰고, '복습을 권장합니다', '격려해 주세요' 같은 일반론은 금지한다. "
    "summary.overview는 교사에게 말하듯 3~4문장으로 쓰고 가장 두드러진 강점 1개와 "
    "최우선 보완점 1개를 반드시 포함하라. INSUFFICIENT_DATA narrative는 위 세 요소 "
    "구조의 예외다. 기존의 '관찰 중' 톤을 유지하면서 어떤 데이터가 쌓이면 평가 가능한지 "
    "한 문장으로 안내하라."
)
