...
- 주장의 조건·가정·적용 범위·예외를 보존하고 가능성과 보장을 구분합니다. 특정 그림의 성질을 모든 경우로 일반화하거나 이전 문맥과 충돌하는 단정을 하지 않습니다.
- 대상의 성질과 절차의 성공 조건·결과의 유일성은 구분합니다. 현재 주장을 한정하는 이전 페이지의 조건은 짧게 연결하되, 근거에서 확인하지 않은 보장이나 다른 페이지 전체 설명을 추가하지 않습니다.
...
- 답변에 필요한 조건·예외를 간결히 밝히고 가능성을 보장으로 바꾸지 않습니다. 자료에 없는 조건을 지어내거나 일반적 면책 문구를 반복하지 않습니다.
- 대상의 성질을 절차의 성공·수렴 보장으로 바꾸지 않으며, 필요한 이전 문맥의 조건은 현재 질문에 연결합니다. 보장에 필요한 조건을 확인할 수 없으면 자료가 직접 뒷받침하는 성질까지만 답합니다.
...
LLM에는 선택한 퀴즈 유형만 포함한 내부 생성 스키마를 전달합니다. 공통
`QuizGeneration`의 정규화·검증을 상속하고, Spring 응답 구조와 5~10개 문항·
해설/루브릭·학습자별 난이도 정책은 유지합니다. 무관한 유형의 스키마 분기만 줄이는
최적화이며, 실제 생성 시간과 학습 품질 개선은 별도 실측으로 확인합니다.
...
- Plan 보정과 완전 거부의 기준
- 모델 선택과 페이지 근거 전달 방식(`pageContext` 텍스트 앵커·폴백 유지, xAI Files는 설명·QA에만 첨부 — DEC-006·035)
