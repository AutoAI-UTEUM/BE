...
- **PDF 접근** — 자료 업로드 시 `/internal/ai/extract`로 1회 추출 → Spring이 `material_pages`와 nullable xAI file ID를 저장 → `/internal/ai/outline`에는 저장된 전 페이지 텍스트를 전달하고, 턴마다 현재±1 페이지 텍스트를 스냅샷에 동봉. Phase 1의 file ID는 수명주기 정리에만 사용하며 턴 첨부는 후속 범위입니다.
...
- 응답: `{ "schemaVersion": "1.0", "pageCount": 42, "pages": [{ "pageNumber": 1, "text": "..." }], "xaiFileId": "file-..." | null, "warnings": [{ "type": "FILE_UPLOAD_FAILED", "message": "..." }] }`. `warnings`는 기본 빈 배열이며 알 수 없는 type도 추출 성공을 무효화하지 않습니다.
