...
| `learning_sessions` | id, user_id, material_id, current_page, page_status, status, conversation_summary, last_ui_actions_json, active_quiz_id, active_turn_request_id, active_turn_started_at, version, timestamps | `FK(user_id)`, `FK(material_id)`, `IDX(user_id,status,updated_at)` |
...
- `learning_sessions.conversation_summary`는 내부 AI 턴 스냅샷 전용이며 외부 세션 상세 응답에는 노출하지 않습니다. `last_ui_actions_json`과 `active_quiz_id`는 재진입 UI 복원용입니다. `active_quiz_id`에는 FK를 추가하지 않습니다. 세션이 퀴즈보다 먼저 생성되는 순환 참조 부담을 피하고 Spring이 퀴즈 생성·제출 상태를 검증합니다.
