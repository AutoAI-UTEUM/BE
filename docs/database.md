...
| `material_overviews` | id, material_id, content(nullable), status, timestamps | `FK(material_id)`, `UK(material_id)`, status CHECK |
...
- `material_overviews`는 자료당 최대 1행이며 `PENDING | READY | FAILED` 상태를 사용합니다. `content`는 nullable이고 조회 응답에서는 READY 상태일 때만 노출합니다. 행이 없는 자료는 API에서 PENDING으로 합성하며, V28은 저장 구조만 추가하고 생성 트리거나 AI 호출은 추가하지 않습니다.
