...
- `V42__xai_management_monitoring.sql`은 `ai_usage_log`에 2차 비용 reconciliation용 nullable `cost_usd_ticks`·`request_id`와 request ID 유일성을 추가하고, 단일 행 `xai_alert_config`를 선반영합니다. 1차에서는 이 비용 컬럼과 alert 설정을 읽거나 쓰지 않습니다.
