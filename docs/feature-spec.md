...
1. 사용자가 이메일, 비밀번호, 이름과 `LEARNER | INSTRUCTOR` 역할을 입력합니다.
2. Spring은 형식, 허용된 공개 가입 역할과 이메일 중복을 검증합니다. `ADMIN`은 공개 가입에서 거부합니다.
3. 비밀번호를 단방향 해시로 저장하고 역할을 포함한 사용자 정보를 반환합니다.

예외: 잘못된 형식, 누락·지원하지 않는 역할, 중복 이메일, 비밀번호 정책 미충족.
