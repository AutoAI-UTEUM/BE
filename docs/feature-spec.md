...
2. Spring은 권한, 콘텐츠 타입·매직 바이트, 크기 제한(45MB·300페이지 — DEC-016)을 검증합니다.
3. 파일을 저장소 어댑터(로컬 볼륨, `storage_key` — DEC-005)에 저장하고 메타데이터와 `PROCESSING` 상태를 저장합니다.
4. Spring이 백그라운드에서 내부 API `POST /internal/ai/extract`로 PDF를 전달하고, FastAPI가 추출한 페이지 텍스트를 반환받아 `material_pages`에 저장합니다(DEC-006).
5. 저장 완료 시 `READY`, 추출 실패 시 `FAILED`로 전환합니다. 처리 중 자료로 세션을 만들 수 없습니다.

### 조회

- 목록은 본인이 업로드한 자료만 반환합니다(소유자 전용 — DEC-026).
- 상세는 페이지 수, 처리 상태 등 학습 시작에 필요한 메타데이터를 반환합니다.
- 페이지 텍스트 API는 운영 FE에 노출하지 않습니다(DEC-025 — dev/디버깅 한정).
...
같은 자료에 기존 ACTIVE 세션이 있으면 그 세션을 반환합니다(재사용 — DEC-024, 응답 `reused: true`). 처음부터 다시 시작하려면 기존 세션을 삭제한 뒤 생성합니다. 페이지 진행은 세션 단일 pageStatus를 유지하며 이동 시 초기화합니다(DEC-008).
...
정답이나 루브릭은 퀴즈 제출 전 FE 응답에 포함하지 않습니다. 기본 문항 수는 5개이며 5~10개 범위 조절은 학습 정책에 따릅니다. 퀴즈 범위는 MVP에서 **현재 페이지 단위**(coverageStart=End=currentPage)를 기본으로 하며, 누적 범위 퀴즈는 확장 항목입니다.
...
채점 판정은 `CORRECT`, `PARTIAL`, `WRONG`만 사용합니다. 재제출은 MVP에서 **한 퀴즈당 1회 제출 제한**으로 확정합니다(DEC-009 Accepted — 재제출 시 `QUIZ_ALREADY_SUBMITTED`). 이후 재제출을 허용하는 확장 시에는 정답 보호 규칙(verdict/feedback 공개 시점·점수 처리·attempt 상한)을 반드시 함께 정의합니다(DEC-009 개선안 참조).
...
통과 기준은 고정 비율 60%(`passed = score/maxScore >= 0.6`, 설정 `EDUPILOT_QUIZ_PASS_RATIO`)입니다(DEC-010 Accepted). 이 기준 미달이 진단 파이프라인의 트리거입니다.
...
1. FastAPI/Grok가 사용자 표시용 진행 요약과 답변 청크를 생성합니다.
2. Spring이 인증된 스트림으로 FE에 중계합니다.
3. FE는 청크를 임시 렌더링합니다.
4. 완료 이벤트 후 Spring이 최종 메시지와 상태를 확정 저장합니다.

연결이 끊기면 미완료 메시지를 확정 메시지로 취급하지 않습니다. 전송 방식은 SSE를 기본으로 하며 재연결/재생·취소·heartbeat 정책은 구현 전 확정합니다.
