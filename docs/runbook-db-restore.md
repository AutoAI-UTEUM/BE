...
4. 내려받은 백업의 `gzip -t`를 서버에서도 확인한다. 이후의 DB 재생성은 **기존
   ...
   않는다. 이 재생성 단계는 필수다. 2026-09-26 리허설에서 옛 덤프에 없는 신규
   테이블이 남아 Flyway 충돌이 실제 발생했으므로, 신규 테이블까지 제거해 덤프 시점
   상태로 맞춘다. 선택한 백업을 복원하고 **파이프라인 종료코드 0**을 확인한다.
...
| 항목 | 기록 |
| --- | --- |
| 실행 일시 / 담당자 | 2026-09-26 / 운영 리허설 |
| 백업 파일명 / S3 크기 | 파일명 미기록 / 3.0 MB |
| 덤프 시점·복원 후 COUNT 5종 | `classrooms=113`, `learning_sessions=571`, `chat_messages=3498`, `exam_submissions=24` 일치 / `users` 미기록 |
| 복원 소요 시간 / 20분 목표 달성 여부 | 전체 복원 시간 미기록 / V41~V48 자동 적용 1.7초 |
| 헬스·Flyway 결과 / dev 원복 여부 | Flyway V41~V48 자동 적용 성공 / 헬스·dev 원복 여부 미기록 |
